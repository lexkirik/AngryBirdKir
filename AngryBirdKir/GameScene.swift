...
//        self.addChild(bird2)
        ...
        let size = CGSize(width: boxTexture.size().width / 5 , height: boxTexture.size().height / 7)
...
//        bird.physicsBody?.affectedByGravity = true
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if let birdPhysic = bird.physicsBody {
            if birdPhysic.velocity.dx <= 0.15 && birdPhysic.velocity.dy <= 0.15 && birdPhysic.angularVelocity <= 0.15 && gameStarted == true {
                bird.physicsBody?.affectedByGravity = false
